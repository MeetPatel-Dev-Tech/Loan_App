...
export default function AddPersonalLoan({navigation, route}) {
  ...
  const [advanceInt, setAdvanceInt] = useState('');
  // const [loanAmount, setAdvanceInt] = useState('');
  const [date, setDate] = useState(new Date());
  const [open, setOpen] = useState(false);
  const [netInfo, setNetInfo] = useState('online');
  const [monthInt, setMonthInt] = useState('');
  const [monthInt2, setMonthInt2] = useState('');
  ...
  const onsubmit = () => {
    navigation.navigate('PersonalLoanLogic', {
      ...
      emiDate: EMIdate,
      type: interest,
      // advanceInt: checkBox == true ? monthInt : 0,
      loanAmount: loanAmount,
      loanType: route.params.loanType,
    });
  };
  const fixdSubmit = () => {
    navigation.navigate('PersonalLoanLogic2', {
      //   appliancename: route.params.appliancename,
      //   brand: route.params.brand,
      //   model: route.params.model,
      //   color: route.params.color,
      ...
      emiDate: EMIdate,
      type: interest,
      // advanceInt: checkBox == true ? monthInt2 : 0,
      loanAmount: loanAmount,
      loanType: route.params.loanType,
    });
  };
  ...
  const verifyedAmount = value => {
    if (
      ...
      if (Number(value) <= Number(Number(loanAmount) / Number(duration))) {
        setError(
          `please enter EMI amt more then ${Number(
            Math.round(Number(loanAmount) / Number(duration)),
          )}`,
        );
      } else {
        setError('');
  ...
  const monthlyinterst = rate => {
    setMonthInt((loanAmount * rate) / 100);
  };
  const advanceINT = loanamount => {
    setMonthInt((loanamount * interstRate) / 100);
  };
  const monthInts = value => {
    if (emiType == 'Interest Only') {
      setMonthInt2(value);
    } else {
      setMonthInt2(
        Number(value) - Number(Number(loanAmount) / Number(duration)),
      );
    }
  };
        ...
        <KeyboardAwareScrollView
          ...
          <View
            ...
            <TextInput
              ...
              onChangeText={value => {
                if (value[0]?.includes(' ')) {
                  setLoanAmount(value.trim());
                } else {
                  setLoanAmount(value.replace(/[^0-9]/, ''));
                  advanceINT(value);
                }
              }}
              label="Loan Amt *"
            ...
            <TextInput
              ...
              onChangeText={value => {
                if (value[0]?.includes(' ')) {
                  ...
                  setProcessingFees(value.replace(/[^0-9]/, ''));
                }
              }}
            ...
            <View style={{}}>
              ...
              {/* {checkBox == true && (
                <View style={{marginTop: 10}}>
                  <Text>
                    {interest == 'percentage'
                      ? `Advance Int. is ${Constant.ruppy} ${monthInt}`
                      : `Advance Int. is ${Constant.ruppy} ${monthInt2}`}
                  </Text>
                </View>
              )} */}
              ...
              {/* <TextInput
                ...
                onChangeText={value => {
                  if (value[0]?.includes(' ')) {
                    setAdvanceInt(value.trim());
                  } else {
                    setAdvanceInt(value.replace(/[^0-9]/, ''));
                  }
                }}
                // onChangeText={value => { setInterstRate(value); }}
                label="Advance int *"
                keyboardType="numeric"
                placeholder="Advance int"
                style={{marginTop: 20}}
                returnKeyType="done"
                ref={RefNo4}
                onSubmitEditing={() => Keyboard.dismiss()}
