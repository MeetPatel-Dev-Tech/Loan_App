...
export default function Logic2({navigation, route}) {
  ...
  const [monthIntrest, setMonthIntrest] = useState(
    route.params.interestType == 'Interest Only'
      ? Number(Math.round(route.params.emiAmount))
      : Number(
          Math.round(
            Number(route.params.emiAmount) -
              Number(loanAmount / route.params.duration),
          ),
        ),
  );
  ...
  const [emiAmount, setEmiAmount] = useState(
    route.params.interestType == 'Interest Only'
      ...
      : Number(
          Math.round(
            (Number(loanAmount) + Number(totalIntrest)) /
  ...
  const [lastEMIAmount, setLastEMIAmount] = useState(
    route.params.interestType == 'Interest Only'
      ? Number(Math.round(Number(loanAmount) + Number(monthIntrest))).toFixed(2)
      : Number(
          Math.round(
            (Number(loanAmount) + Number(totalIntrest)) /
              Number(route.params.duration),
  ...
  // const [interstRate, setInterstRate] = useState(route.params.interestType == 'Interest Only' ? Number(route.params.emiAmount * 100) / loanAmount :Number(monthIntrest * 100) / loanAmount);
  const [interstRate, setInterstRate] = useState(
    Number(Math.round(Number(monthIntrest * 100) / Number(loanAmount))),
  );
  ...
  const onSubmitData = async (invoice, product) => {
    ...
    if (netInfo == 'online') {
      const data = {
        ...
        downPayment: Number(route.params.downPaymentAmount),
        loanAmount: Number(loanAmount),
        emiType: route.params.interestType == 'Interest Only' ? 0 : 1,
        emiRate: Number(interstRate),
          ...
          <View style={{flex: 1, marginHorizontal: 20, marginTop: 40}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <View style={{alignItems: 'flex-end'}}>
                ...
                {route.params.interestType == 'Interest Only' ? (
                  <Text style={{marginTop: 20}}>{interstRate}%</Text>
                ) : (
                  <Text style={{marginTop: 20}}>{interstRate}%</Text>
                )}
