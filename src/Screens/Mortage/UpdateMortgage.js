...
export default function UpdateMortgage({navigation, route}) {
  const [accountHolderName, setAccountHolderName] = useState(
    route.params?.MortgageDetails?.accountHoldername,
  );
  const [checkNumber, setcheckNumber] = useState(
    route.params?.MortgageDetails.checkNumber == null
      ? ''
      : route.params?.MortgageDetails.checkNumber,
  );
  const [bankName, setBankName] = useState(
    route.params?.MortgageDetails.bankName,
  );
  const [accountNumber, setAccountNumber] = useState(
    route.params?.MortgageDetails.accountNumber,
  );
